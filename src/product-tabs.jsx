...
class ProductTabs extends React.PureComponent {
  ...
  render() {
    return (
      <React.Fragment>
        <div className="tab">
          <button className={`tablinks tab-text ${this.state.selectedTab == 'description' ? 'highlight' : ''}`} onClick={this.setDescription}>Description</button>
          <button className={`tablinks tab-text ${this.state.selectedTab == 'details' ? 'highlight' : ''}`} onClick={this.setDetails}>Details</button>
                </div>
                <div className='product-desc-tabs--divider'></div>
